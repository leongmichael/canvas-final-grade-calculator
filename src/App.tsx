...
function App() {
  ...
  const checkPage = () => {
    chrome.tabs.query({ active: true, currentWindow: true }, (tabs) => {
      ...
      if (activeTab && activeTab.id !== undefined && activeTab.url && activeTab.url.includes("sjusd.instructure.com")) {
        setIsOnSjusdPage(true);
        // Ensure activeTab.id is defined before using it
        chrome.tabs.sendMessage(activeTab.id, { message: "getGrade" }, (response) => {
          if (response) {
